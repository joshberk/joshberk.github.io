...
console.log('✓ Removed duplicate nonces');
console.log('\nNOTE: This nonce should be regenerated for each page request in a real server environment.');

// Verification step - check for issues
console.log('\n=== VERIFICATION ===');
htmlFiles.forEach((file) => {
  if (fs.existsSync(file)) {
    const content = fs.readFileSync(file, 'utf8');
    const scriptTags = content.match(/<script[^>]*>/g) || [];
    const nonceCount = (content.match(/nonce="/g) || []).length;
    const duplicateNonces = content.match(/nonce="[^"]*"\s*nonce="/g);
    
    console.log(`${file}:`);
    console.log(`  - Script tags: ${scriptTags.length}`);
    console.log(`  - Nonce attributes: ${nonceCount}`);
    if (duplicateNonces) {
      console.log(`  - ⚠️  DUPLICATE NONCES FOUND: ${duplicateNonces.length}`);
    } else {
